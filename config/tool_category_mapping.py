...
TOOL_TO_CATEGORY = {
    ...
    # 逻辑漏洞测试工具
    "idor_enum": "web_exploitation",
    "idor": "web_exploitation",  # 别名
    "race_condition": "web_exploitation",
    "race": "web_exploitation",  # 别名
    "param_tamper": "web_exploitation",
    "param-tamper": "web_exploitation",  # 别名
    "auth_bypass": "web_exploitation",
    "auth-bypass": "web_exploitation",  # 别名
...
CATEGORY_DESCRIPTIONS = {
    ...
    "web_exploitation": "Web漏洞利用工具，包括SQL注入、XSS、文件上传、逻辑漏洞（IDOR越权、竞争条件、参数篡改、认证绕过、业务流程绕过）等",
...
def get_tool_category_mapping_text():
    ...
    if MULTI_CATEGORY_TOOLS:
        ...
        for tool, categories in sorted(MULTI_CATEGORY_TOOLS.items()):
            ...
            lines.append(
                f"  - {tool}: 主要类别={main_category}, 也可用于={', '.join(categories)}"
            )

    lines.append("\n\n⚠️ 分类规则（按优先级）：")
    lines.append(
        "1. 【最高优先级】如果用户明确提到了工具名称（如'使用nmap扫描'、'用sqlmap测试'），必须根据上述映射选择对应的target"
    )
    lines.append("2. 工具名称不区分大小写（如nmap/Nmap/NMAP都视为同一工具）")
    lines.append(
        "3. 如果工具名称有多种写法，参考别名（如metasploit/msf/msfconsole都指向同一工具）"
    )
    lines.append("4. 对于多类别工具，按以下规则处理：")
    lines.append("   - 优先选择映射表中的主要类别")
    lines.append("   - 如果任务描述中包含明确的场景关键词，选择匹配的类别")
    lines.append(
        "   - 例如：'用gobuster枚举目录' → enumeration，'用gobuster扫描端口' → scanning"
    )
    lines.append(
        "   - 例如：'用hydra爆破SSH' → exploitation，'用hydra破解密码' → password_crypto"
    )
