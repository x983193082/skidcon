...
from skidc.dispatcher.config import WorkerConfig, resolve_mock_behavior
from skidc.dispatcher.workers.base import DriverResult, SeedSessionDriver

# A self-contained python script the mock worker runs inside the container (or as a
# local subprocess in tests). It reads a behavior spec + the structured prompt, sleeps
# a random delay, then emits a contract-valid (or deliberately invalid) JSON payload so
# the dispatcher's success / failure / timeout paths can be exercised without any LLM.
_SCRIPT = """
