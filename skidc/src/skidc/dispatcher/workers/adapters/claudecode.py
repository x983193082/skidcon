...
from skidc.dispatcher.workers.base import DriverResult, SeedSessionDriver
...
class ClaudeCodeDriver(SeedSessionDriver):
    """Drives the `claude` CLI against any Anthropic-compatible endpoint.

    In Skidc's reference config this points at DeepSeek's Anthropic-compatible API
    (ANTHROPIC_BASE_URL=https://api.deepseek.com/anthropic, ANTHROPIC_MODEL=deepseek-...),
    so the agent loop is Claude Code but the brain is DeepSeek. Nothing here is
    Anthropic-specific beyond the wire protocol."""
    ...
    def build_healthcheck(self, worker: WorkerConfig) -> list[str]:
        ...
        return [
            ...
            "-H", f"Authorization: Bearer {env['ANTHROPIC_AUTH_TOKEN']}",
    ...
    def describe_startup_healthcheck(self, worker: WorkerConfig) -> str:
        ...
        return render_curl_command(
            f"{env['ANTHROPIC_BASE_URL']}/v1/messages",
            headers=[
                "-H", expand_env("Authorization: Bearer $ANTHROPIC_AUTH_TOKEN"),
                "-H", f"anthropic-version: {ANTHROPIC_VERSION}",
                "-H", "content-type: application/json",
            ],
            payload=(
