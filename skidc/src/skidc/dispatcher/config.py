...
# Each backend reads its model + endpoint + key purely from env. This is the seam
# that lets a "claudecode" worker actually drive DeepSeek, and a "codex" worker drive Qwen.
...
MOCK_DEFAULT_BEHAVIOR: dict[str, dict[str, Any]] = {
    ...
    "explore_execute": {
        "delay": [0.05, 0.3],
        "outcomes": {
            "fact": "1.0", "rejected": "0.0", "invalid_json": "0.0",
...
def validate_prompt_resources(prompt_group: str) -> None:
    prompts_dir = resources.files("skidc.dispatcher.prompts")
...
def resolve_mock_behavior(worker_name: str, env: dict[str, str]) -> dict[str, dict[str, Any]]:
    ...
    for phase, allowed_outcomes in MOCK_ALLOWED_OUTCOMES.items():
        ...
        behavior[phase] = {"delay": {"min": min_delay, "max": max_delay}, "outcomes": outcomes}
        rules = payload.get("rules")
        if rules is not None:
            behavior[phase]["rules"] = _parse_mock_rules(worker_name, prefix, rules, allowed_outcomes)
    return behavior


def _parse_mock_rules(worker_name: str, prefix: str, rules: Any, allowed: frozenset[str]) -> list[dict[str, Any]]:
    ...
    for index, rule in enumerate(rules):
        ...
        if not isinstance(force, str) or force not in allowed:
            raise ValueError(f"worker {worker_name} {prefix}.rules[{index}].force must be one of: {', '.join(sorted(allowed))}")
        entry: dict[str, Any] = {"force": force}
        for key in ("fact_ids_gte", "fact_ids_lte"):
            if key in rule:
                value = rule[key]
                if not isinstance(value, int) or value < 0:
                    raise ValueError(f"worker {worker_name} {prefix}.rules[{index}].{key} must be a non-negative integer")
                entry[key] = value
