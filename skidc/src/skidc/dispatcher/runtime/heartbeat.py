...
from skidc.dispatcher.protocol.client import ApiResult, SkidcClient
from skidc.dispatcher.runtime.process import ManagedProcess
...
class HeartbeatLease:
    """Background thread that renews a claim (intent or reason lease) on a cadence.
    If the server hard-rejects the claim (403/409) or the lease lapses past its grace
    window, the lease marks itself failed and kills the worker process it's attached to."""
