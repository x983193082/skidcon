...
3. **Level 3 Agent (工具执行器)**: 每个工具都有专门的执行Agent，直接在Kali Linux中执行命令

## 功能特性

- ✅ 三层Agent自动路由和执行
- ✅ 支持50+渗透测试工具（nmap, sqlmap, metasploit等）
- ✅ 直接在 Kali Linux 环境中执行（无需Docker）
- ✅ 流式输出，实时查看执行过程
- ✅ 支持多种安全测试领域（信息收集、扫描、枚举、Web利用等）
- ✅ 现代化Vue3+Vite前端界面
- ✅ 使用OpenRouter API，支持多种LLM模型

## 技术栈

- **AI框架**: CrewAI 0.86.0
- **LLM**: OpenRouter API (默认模型: z-ai/glm-5.1)
- **前端**: Vue 3 + Vite
- **后端**: FastAPI + Uvicorn
