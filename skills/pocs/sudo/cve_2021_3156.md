...
description: 针对 CVE-2021-3156 的详细分析与 PoC 触发技能。当你具备sudo版本在1.8.2到1.9.5p1之间，并且有sudoedit命令可用时，使用此SKILL。
---
验证漏洞存在方法
    - 检查sudo版本：`sudo --version | head -1`
    - 测试漏洞是否存在：`sudoedit -s '\\'` 或使用专门的检测脚本
    - 如果系统存在漏洞，命令可能会崩溃或显示错误信息

漏洞利用方法
    - 这是一个sudo中的堆缓冲区溢出漏洞，可以通过sudoedit -s命令和以单个反斜杠结尾的命令行参数来触发，允许本地用户提升权限到root
    - 漏洞影响sudo的legacy版本从1.8.2到1.8.31p2，稳定版本从1.9.0到1.9.5p1
    - 利用该漏洞需要构造特定的参数和环境变量来触发堆溢出，从而覆盖关键内存区域
