...
description: 针对 CVE-2024-23897 的详细分析与 PoC 触发技能。当你具备存在Jenkins 2.441及更早版本或LTS 2.426.2及更早版本时，使用此SKILL。
---
验证漏洞存在方法
    - 检查Jenkins版本是否为2.441及更早版本或LTS 2.426.2及更早版本
    - 尝试通过CLI接口发送包含'@'字符和文件路径的命令

漏洞利用方法
    - Jenkins CLI命令解析器存在缺陷，当参数中包含'@'字符后跟文件路径时，会将该文件内容替换到参数中
    - 攻击者可以利用此特性读取Jenkins控制器文件系统上的任意文件
    - 
```python
