---
name: poc-samba-cve_2017_7494
description: 针对 CVE-2017-7494 的详细分析与 PoC 触发技能。当你具备Samba版本在3.5.0到4.6.4、4.5.10、4.4.14之间，并且有可写的共享目录时，使用此SKILL。
---
验证漏洞存在方法
    - 检查目标Samba服务器版本是否在受影响范围内（3.5.0到4.6.4、4.5.10、4.4.14之间）
    - 尝试连接到Samba服务器并列出可用的共享目录
    - 检查是否有可写的共享目录
    - 尝试上传测试文件到可写共享以验证写入权限

漏洞利用方法
    - 该漏洞允许恶意客户端上传共享库到可写共享，然后使服务器加载并执行它
    - 攻击者需要创建一个恶意的共享库（如.so文件），其中包含要执行的代码
    - 使用SMB协议连接到目标服务器，找到可写的共享目录
    - 将恶意共享库上传到可写共享
    - 通过IPC$管道触发服务器加载上传的共享库
    - 服务器将以root权限执行共享库中的代码
