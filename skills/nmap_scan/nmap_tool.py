...
class NmapScanInput(BaseModel):
    ...
    ports: str = Field(
        default=None, description="端口范围，如 '1-1000' 或 '80,443,8080'"
    )
    scan_type: str = Field(
        default="-sV", description="扫描类型: -sV版本检测, -sS SYN扫描, -sT TCP扫描"
    )
    extra_args: str = Field(default="", description="额外参数")


class QuickNmapInput(BaseModel):
    """快速扫描输入参数"""

    target: str = Field(description="目标主机 IP或域名")


class FullNmapInput(BaseModel):
    """全面扫描输入参数"""
...
class QuickNmapTool(BaseTool):
    ...
    description: str = (
        "快速扫描目标主机的常用端口，适合初步信息收集。必填参数: target(目标)"
    )
    args_schema: Type[BaseModel] = QuickNmapInput
...
class FullNmapTool(BaseTool):
    ...
    description: str = (
        "对目标进行全面扫描，包括操作系统检测和漏洞脚本扫描。必填参数: target(目标)"
    )
    args_schema: Type[BaseModel] = FullNmapInput
